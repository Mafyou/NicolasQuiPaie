...
public class JwtServiceTests
{
    ...
    [
        new("user-petit", "petit@nicolas.fr", "Petit Nicolas", InfrastructureFiscalLevel.PetitNicolas, 100),
        new("user-moyen", "moyen@nicolas.fr", "Moyen Nicolas", InfrastructureFiscalLevel.GrosMoyenNicolas, 250),
        new("user-gros", "gros@nicolas.fr", "Gros Nicolas", InfrastructureFiscalLevel.GrosNicolas, 500),
        new("user-supreme", "supreme@nicolas.fr", "Nicolas Suprême", InfrastructureFiscalLevel.NicolasSupreme, 1000),
        new("user-unverified", "unverified@nicolas.fr", "Unverified User", InfrastructureFiscalLevel.PetitNicolas, 50, false)
    ];
    ...
    public void GenerateToken_ShouldCreateValidJwt_ForAllUserTypes(TestUserData userData)
    {
        ...
        var user = new ApplicationUser
        {
            ...
            FiscalLevel = userData.FiscalLevel,
        ...
        fiscalLevelClaim.ShouldBe(userData.FiscalLevel.ToString());
    ...
    // C# 13.0 - Params collections for flexible claim testing
    [Test]
    public void GenerateToken_ShouldIncludeExpectedClaims_InCorrectFormat(
        params string[] expectedClaimTypes)
    {
        ...
        var user = TestDataHelper.CreateTestUser(
            "claim-test-user",
            "claims@test.fr",
            InfrastructureFiscalLevel.GrosNicolas);

        // Define expected claims with C# 13.0 collection expressions
        ...
        var tokenHandler = new JwtSecurityTokenHandler();
        var decodedToken = tokenHandler.ReadJwtToken(token);
        var actualClaimTypes = decodedToken.Claims.Select(c => c.Type).ToArray();

        // C# 13.0 - Enhanced collection assertions with modern null checking
        foreach (var requiredClaim in requiredClaims)
        {
            actualClaimTypes.ShouldContain(requiredClaim, 
                $"Token should contain {requiredClaim} claim");
        }
    }
    ...
    [
        ...
        [new ApplicationUser { Id = "", Email = "test@test.com" }, typeof(ArgumentException), "User ID"],
        [new ApplicationUser { Id = "valid-id", Email = "" }, typeof(ArgumentException), "Email"]
    ];
    ...
    public void ValidateToken_ShouldReturnClaimsPrincipal_ForValidToken()
    {
        ...
        var user = TestDataHelper.CreateTestUser(
            "validate-user",
            "validate@test.fr",
            InfrastructureFiscalLevel.NicolasSupreme);
        ...
        fiscalLevelClaim.ShouldBe(user.FiscalLevel.ToString());
    }
    ...
    public void ValidateToken_ShouldThrowException_ForInvalidTokens(
        string invalidToken, string expectedErrorPattern)
    {
        // Act & Assert
        var exception = Should.Throw<SecurityTokenException>(
    ...
    [Test]
    public void GetTokenExpiryDate_ShouldReturnCorrectExpiry_ForValidToken()
    {
        // Arrange
        var user = TestDataHelper.CreateTestUser();
        var beforeGeneration = DateTime.UtcNow;
        ...
        // Act
        var expiryDate = _jwtService.GetTokenExpiryDate(token);

        // Assert
        var expectedExpiry = beforeGeneration.AddMinutes(60);
        Math.Abs((expiryDate - expectedExpiry).TotalMinutes).ShouldBeLessThan(1);
    }
    ...
    public async Task GenerateRefreshToken_ShouldCreateUniqueTokens_ForMultipleUsers()
    {
        ...
        var users = TestUsers.Take(3).Select(userData => 
            TestDataHelper.CreateTestUser(userData.Id, userData.Email, userData.FiscalLevel)).ToArray();
