...
public class VotingEndpointsTests
{
    ...
    public async Task PostVote_ShouldCalculateCorrectWeight_BasedOnFiscalLevel()
    {
        ...
        var testCases = new[]
        {
            new { Token = GetTestJwtToken(FiscalLevel.PetitNicolas), ExpectedWeight = 1 },
            new { Token = GetTestJwtToken(FiscalLevel.GrosMoyenNicolas), ExpectedWeight = 2 },
            new { Token = GetTestJwtToken(FiscalLevel.GrosNicolas), ExpectedWeight = 3 },
            new { Token = GetTestJwtToken(FiscalLevel.NicolasSupreme), ExpectedWeight = 5 }
        };
    ...
    /// </summary>
    private static string GetTestJwtToken(FiscalLevel fiscalLevel = FiscalLevel.PetitNicolas)
    {
